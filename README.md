# tf-aws-module_primitive-elasticache_parameter_group
...
This primitive module creates an AWS ElastiCache parameter group (`aws_elasticache_parameter_group`) so Redis or Memcached clusters can share a named set of engine settings. Provide a group name and parameter family, then optionally set a description, tags, and a list of parameter name/value pairs. The module returns the parameter group's ARN, name, family, and applied parameters for use by ElastiCache cluster or replication-group modules.
